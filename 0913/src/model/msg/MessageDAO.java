...
public class MessageDAO {
	...
	public ArrayList<MsgSet> selectAll(String u_id, int cnt) {
		...
		try {
			...
			if((u_id == null) || u_id.equals("")) {
				sql = "select * from messages where rownum <= ? order by mdate desc";
				pstmt = conn.prepareStatement(sql);
				pstmt.setInt(1, cnt);
			} else { // 특정 회원 댓글 조회
				sql = "select * from messages where u_id = ? and rownum <= ? order by mdate desc";
				pstmt = conn.prepareStatement(sql);
	...
	public boolean insert(MessageVO invo) {
		conn = JNDI.getConnection();
		String sql = "insert into messages (m_id, u_id, msg) values (nvl(0, (select max(m_id) from messages)+1),?,?)";
	...
	public boolean delete(MessageVO invo) {
		conn = JNDI.getConnection();
	...
	// 좋아요+1
	public void update(MessageVO invo) {
		conn = JNDI.getConnection();
